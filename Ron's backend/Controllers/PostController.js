...
class PostController{
    ...
    static async getMyPosts(req, res){
        try{
            ...
            const myPosts = await Post.find({"postOwner._id":user._id});
    ...
    static async comentPost(req, res){
        ...
        try{
            ...
            res.status(200).json({message: "Comentário feito", newPost});
        }catch(err){
    ...
    static async likedPost(req, res){
        ...
        try{
            const newPost = await post.save();
            res.status(200).json({message: msg, newPost});
        }catch(err){
